...
def _ext_prepare_train_features_flatten_trunc(examples, tokenizer):
    ...
    tokenized_q = tokenizer(examples['question'], return_tensors='pt',
                            truncation=True, max_length=384, padding="max_length")
    tokenized_c = tokenizer(examples['context'], return_tensors='pt', truncation=True, max_length=384,
                            stride=128, return_overflowing_tokens=True, return_offsets_mapping=True, padding="max_length")
    tokenized_a = tokenizer(texts, return_tensors='pt',
...
def mask_word_with_ST(train_dataset, tokenizer):
    ...
    for i, input_id in tqdm(enumerate(train_dataset['input_ids'])):
        ...
        if tokenizer.pad_token_id in np.array(input_id):
            pad_idx = np.where(np.array(input_id) ==
                               tokenizer.pad_token_id)[0][0]

        # sep_idx[0][0]: the position of first [SEP] token
        question = tokenizer.decode(input_id[1:sep_idx])
        answer = tokenizer.decode(
            input_id[train_dataset['start_positions'][i]:train_dataset['end_positions'][i]+1])
    ...
    with tqdm(dataloader, unit="batch") as tepoch:
        for batch in tepoch:
            ...
            answers = [tokenizer.decode(
                i, skip_special_tokens=True) for i in batch[6]]
            ...
            p_outputs = torch.transpose(
                p_outputs.view(batch_size, 384, -1), 1, 2)
            ...
            for idx, score in enumerate(sim_scores):
                ...
                for i in range(2):
                    p_inputs['input_ids'][idx] = make_mask_word(
                        tokenizer, p_inputs['input_ids'][idx], answers[idx], sorted_idx[i])
                new_ids.append(p_inputs['input_ids'][idx].tolist())

    origin_answers = train_dataset['answers']
    origin_start = [data['answer_start'][0]
                    for data in train_dataset['answers']]
            ...
            if offsets[0] == offsets[1]:
                ...
                if new_ids[list_idx][idx] == tokenizer.mask_token_id:
                    for i in range(offsets[0], offsets[1]):
                        ...
                        origin_text[origin_list_idx] = list(
                            origin_text[origin_list_idx])
                        origin_text[origin_list_idx][i] = '∬'
                        origin_text[origin_list_idx] = "".join(
                            origin_text[origin_list_idx])

    for i, text in enumerate(origin_text):
        origin_text[i] = text[:origin_start[i]] + \
            '[ans]' + text[origin_start[i]:]
        ...
        for i, batch in enumerate(tepoch):
            ...
            for idx, score in enumerate(sim_scores):
                ...
                for word in sim_words:
                    ...
                    origin_text[origin_idx] = re.sub(
                        re.escape(word), word*2, origin_text[origin_idx])
                    origin_text[origin_idx] = re.sub(
                        re.escape(word*4), word*2, origin_text[origin_idx])
...
def get_question_random_masking_dataset(train_data_path, save_path):
    ...
    train_dataset = load_from_disk(os.path.join(
        train_data_path, "train_dataset"))['train']
    val_dataset = load_from_disk(os.path.join(
        train_data_path, "train_dataset"))['validation']
...
def get_ST_mask_dataset(train_data_path, save_path):
    ...
    raw_train_dataset = load_from_disk(os.path.join(
        train_data_path, "train_dataset"))['train']
    raw_val_dataset = load_from_disk(os.path.join(
        train_data_path, "train_dataset"))['validation']
    column_names = raw_train_dataset.column_names

    _ext_prepare_train_features = partial(
        get_extractive_features, tokenizer=tokenizer, mode="train")
    ...
    _ext_prepare_validation_features = partial(
        get_extractive_features, tokenizer=tokenizer, mode="eval")
...
def get_emb_mask_dataset(dataset_path, mode, save_path):
    ...
    raw_train_dataset = load_from_disk(
        os.path.join(dataset_path, "train_dataset"))['train']
    raw_val_dataset = load_from_disk(os.path.join(
        dataset_path, "train_dataset"))['validation']

    tokenized_c = tokenizer(raw_train_dataset['context'], return_tensors='pt', truncation=True, max_length=384,
                            stride=128, return_overflowing_tokens=True, return_offsets_mapping=True, padding="max_length")

    offset_mapping = tokenized_c.pop("offset_mapping")
    sample_mapping = tokenized_c.pop("overflow_to_sample_mapping")

    ext_prepare_train_features_flatten_trunc = partial(
        _ext_prepare_train_features_flatten_trunc, tokenizer=tokenizer)
    ...
    Tensor_dataset = TensorDataset(
        ...
        torch.tensor(new_q['ids']), torch.tensor(
            new_q['attention']), torch.tensor(new_q['token_type']),
    ...
    if mode == "mask":
        masked_dataset = mask_word_with_emb(
            train_dataloader, tokenizer, offset_mapping, sample_mapping, raw_train_dataset)
    elif mode == "hard":
        masked_dataset = add_word_with_emb(
            train_dataloader, tokenizer, offset_mapping, sample_mapping, raw_train_dataset)
